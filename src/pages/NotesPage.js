    ...
    <div style={{ flex: 1, overflow: 'auto', padding: '0 0 80px' }}>
      ...
      <div style={{ padding: '12px 16px' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-3)' }}>
            ...
            <div style={{ fontSize: '14px', fontWeight: '500', marginBottom: '6px' }}>No memos yet</div>
            <div style={{ fontSize: '12px' }}>Tap "+ Add memo" to write your first memo.</div>
          </div>
        ) : filtered.map(note => {
          const fac = facilities.find(f => f.id === note.facility_id);
          const linkedItem = note.item_id ? items.find(i => i.id === note.item_id) : null;
          return (
        ...
        <div className="overlay overlay-center" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="sheet-center" style={{ padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: '600' }}>Add memo</h2>
            ...
            <div className="form-actions">
              ...
              <button className="btn btn-sm btn-primary" onClick={handleSave}>Save memo</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingNote && (
        <div className="overlay overlay-center" onClick={e => e.target === e.currentTarget && setEditingNote(null)}>
          <div className="sheet-center" style={{ padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: '600' }}>Edit memo</h2>
